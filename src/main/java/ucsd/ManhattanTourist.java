...
public class ManhattanTourist {
     ...
     */
    public static int getLongestPathLength(int n, int m, int[][] downMatrix, int[][] rightMatrix) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int j = 1; j <= m; j++) {
                lengthMatrix[i][j] =
                        Math.max(lengthMatrix[i - 1][j] + downMatrix[i - 1][j],
    ...
    public static String doWork(String dataFileName) {
        try (BufferedReader br = new BufferedReader(new FileReader(dataFileName))) {
            ...
            int longestPathLength = getLongestPathLength(n, m, downMatrix, rightMatrix);
    ...
    public static void main(String[] args) throws IOException {
        String result = doWork("src/test/resources/III/dataset_261_9.txt");

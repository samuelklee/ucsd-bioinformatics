...
public class FittingAlignment {
            ...
            for (int j = 1; j <= w.length(); j++) {
                List<Integer> scores = Arrays.asList(
                        ...
                        v.charAt(i - 1) == w.charAt(j - 1) ?
                                lengthMatrix[i - 1][j - 1] + 1 : lengthMatrix[i - 1][j - 1] - 1);
                lengthMatrix[i][j] = Collections.max(scores);
                if (j == w.length() && lengthMatrix[i][j] >= maxLength) {
    ...
    public static int getScoreFromBacktrack(Action[][] backtrackMatrix, String v, String w, int i, int j) {
        ...
        switch (action) {
            case DELETION:
                return getScoreFromBacktrack(backtrackMatrix, v, w, i - 1, j) - 1;
            case INSERTION:
                return getScoreFromBacktrack(backtrackMatrix, v, w, i, j - 1) - 1;
            case MATCH:
...
//            System.out.println(getActionsFromBacktrack(backtrackMatrix, iMax, jMax));
    ...
    public static void main(String[] args) throws IOException {
        String result = doWork("src/test/resources/III/dataset_248_5.txt");
